  ...
  (
    {
      width = 840,
      height = 640,
      zoomExtent = [0.5, 20] as [number, number],
      maxLinkedPoint = 2,
      data,
      className,
    },
    ...
    const quadtreeRef = useRef<Quadtree<Point>>(null);
    ...
    const canvasDraw = useCallback(() => {
      ...
      for (const d of visiblePoints) {
        ...
        ctx.beginPath();
        ctx.arc(x, y, d.r * transform.k, 0, Math.PI * 2);
        // Hover点设置

        if (hoveredPoint && d.id === hoveredPoint.id) {
          continue
        }
        // 普通点设置

        if (hasSelectedPoint) {
          ctx.strokeStyle = "oklch(0.13 0.028 261.692 / 30%)";
          ctx.fillStyle = "oklch(0.13 0.028 261.692 / 10%)";
        } else {
          ctx.strokeStyle = "oklch(72.3% .219 149.579 / 80%)";
          ctx.fillStyle = "oklch(72.3% .219 149.579 / 20%)";
        }
      ...
      if (hoveredPoint!==null) {
        const { x: hx, y: hy, r } = hoveredPoint;
        // 应用当前变换对点坐标
        const [x, y] = transform.apply([hx, hy]);

        ctx.beginPath();
        ctx.arc(x, y, r * transform.k, 0, Math.PI * 2);
        // Hover点设置
      ...
      if (selectedPoints.length === 2) {
        const [p1x, p1y] = transform.apply([
          selectedPoints[0].x,
          selectedPoints[0].y,
        ]);
        const [p2x, p2y] = transform.apply([
          selectedPoints[1].x,
          selectedPoints[1].y,
        ]);
        ctx.beginPath();
        ...
        ctx.moveTo(p1x, p1y);
        ctx.setLineDash([2 * transform.k, 5 * transform.k]);
      ...
      for (const d of visibleSelectedPoints) {
        ...
        ctx.beginPath();
        ctx.arc(x, y, d.r * transform.k * 1.2, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc(x, y, d.r * transform.k * 0.8, 0, Math.PI * 2);
        ctx.fill();
      ...
      ctx.restore();
    }, [width, height, selectedPoints, hoveredPoint, transform]);

    const throttleZoomed = throttle(
    ...
    const initBrush = () => {
      ...
      const brushBehavior = brush<any>()
        ...
        if(brushBehavior)
        select(brushRef.current).call(brushBehavior);
    ...
    const updateAxis = () => {
      ...
      // transform过后的转换函数
      const scaleX = transform.rescaleX(xScale).interpolate(interpolateRound);
      const scaleY = transform.rescaleY(yScale).interpolate(interpolateRound);

      // 定义xy的最大刻度数
      const xTicks =
        k > 1 ? Math.max(4, Math.floor(9 / k)) : Math.min(9, Math.floor(9 / k));
      const yTicks =
        k > 1 ? Math.max(2, Math.floor(6 / k)) : Math.min(6, Math.floor(6 / k));
    ...
    const handleMouseMove = (event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if ((!closestPoint && hoveredPoint) || (closestPoint && !hoveredPoint)) {
        if (selectedPoints.length === 0) setHoveredPoint(closestPoint);
      }
    };
    ...
    useEffect(() => {
      ...
      updateAxis()
      canvasDraw()
       brushBehavior.current = initBrush();
    ...
    return (
      <div className={`w-[100vw] relative pa-1 ${className}`}>
        <canvas
          ref={canvasRef}
          style={{ position: "absolute", inset: 60, left: 80,    zIndex:brushActive ? 1 : 10,       pointerEvents: brushActive ? "none" : "all", }}
          onClick={handleClick}
